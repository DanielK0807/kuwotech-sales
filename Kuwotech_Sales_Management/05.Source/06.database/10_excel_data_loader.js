...
export class ExcelDataLoader {
  ...
  async loadExcelFile(file, options = {}) {
    ...
    try {
      ...
      // KPI 재계산 트리거
      // 데이터 로드가 모두 완료된 후, 비동기적으로 KPI 재계산을 시작합니다.
      // { quiet: true } 옵션으로 사용자에게 별도 토스트 메시지를 띄우지 않습니다.
      kpiManager.triggerKpiRecalculation({ quiet: true });
    }
  }
  ...
  parseCompanyRow(row, rowNum) {
    try {
      ...
      if (!keyValue && no) {
        ...
        this.warnings.push(
          `행 ${rowNum}: KEY VALUE 자동 생성 (NO 기반) - ${keyValue}`
        );
      } else if (!keyValue) {
  ...
  parseEmployeeRow(row, rowNum) {
    try {
      // 이름 확인
      const name =
        row["이름"] || row["직원명"] || row["성명"] || Object.values(row)[0];
      if (!name || !name.trim()) {
        return null;
      }
        ...
        try {
          ...
          if (existing) {
            ...
            if (!matchedByKeyValue) {
              logger.info(
                `[거래처 업데이트] 거래처명 매칭: ${company.finalCompanyName} → ${existing.keyValue}`
              );
            }
          } else {
            // 새로 추가
            ...
            addedCount++;
            logger.info(
              `[거래처 추가] 신규: ${company.finalCompanyName} → ${company.keyValue}`
            );
          }
        } catch (error) {
          errorCount++;
          ...
          this.errors.push(
            `거래처 ${company.finalCompanyName} 저장 실패: ${error.message}`
          );
        }
      }

      logger.info(
        `[거래처 저장 완료] 추가: ${addedCount}, 업데이트: ${updatedCount}, 오류: ${errorCount}`
      );
    } catch (error) {
    ...
    try {
      ...
      for (const employee of this.loadedData.employees) {
        try {
          ...
          this.warnings.push(
            `직원 ${employee.name} 저장 실패: ${error.message}`
          );
        }
      }
      ...
      sessionStorage.setItem(
        "employees_data",
        JSON.stringify(this.loadedData.employees)
      );
    } catch (error) {
  ...
  getSummary() {
    return {
      companies: {
        ...
        active: this.loadedData.companies.filter(
          (c) => c.transactionStatus === "활성"
        ).length,
        inactive: this.loadedData.companies.filter(
          (c) => c.transactionStatus !== "활성"
        ).length,
      },
      employees: {
        total: this.loadedData.employees.length,
        sales: this.loadedData.employees.filter((e) => e.role?.includes("영업"))
          .length,
        admin: this.loadedData.employees.filter((e) =>
          e.role?.includes("관리자")
        ).length,
      },
