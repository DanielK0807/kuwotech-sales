...
async function initExcelUpload() {
  try {
    ...
    // 필수 요소가 없으면 에러
    if (
      !requiredElements["upload-zone"] ||
      !requiredElements["excel-file-input"]
    ) {
...
async function loadCurrentStatus() {
  try {
    ...
    document.getElementById("current-count").textContent = `${formatNumber(
      count
    )}개사`;
    ...
    if (count > 0) {
      ...
      const lastUpdate =
        sortedCompanies.length > 0 ? sortedCompanies[0].updatedAt : null;

      document.getElementById("last-upload").textContent = lastUpdate
...
function setupEventListeners() {
  ...
  if (removeFileBtn && fileInput) {
    removeFileBtn.addEventListener("click", () => {
      ...
      if (uploadZoneEl) uploadZoneEl.classList.remove("hidden");
      if (previewDiv) {
...
async function handleFileSelect(file) {
  try {
    ...
    document.getElementById("file-date").textContent = formatDate(
                    ...
                    <div class="text-sm text-secondary max-h-100 overflow-y-auto">
                        ...
                        ${
                          stats.warnings.length > 5
                            ? `<br>• ... 외 ${stats.warnings.length - 5}개`
                            : ""
                        }
                    </div>
                </div>
...
function analyzeData(data) {
  ...
  const hasCompanyName =
    columns.includes("거래처명(ERP)") || columns.includes("최종거래처명");
  ...
  data.forEach((row, index) => {
    ...
    if (isEmpty && index < data.length - 5) {
      // 마지막 5행은 제외
...
function showChanges(result) {
  ...
  document
    .getElementById("cancel-changes-btn")
...
function generateChangeDetails(changes) {
  ...
  if (changes.added && changes.added.length > 0) {
    ...
    if (changes.added.length > 10) {
      html += `<div class="mt-xs" style="color: rgba(255, 255, 255, 0.7);">... 외 ${
        changes.added.length - 10
      }개</div>`;
    }
    ...
    changes.modified.slice(0, 5).forEach((item) => {
      ...
      html += `
                <div class="bg-white-05 p-sm border-radius-sm">
                    ...
                    ${
                      item.changes
                        ? `<div class="text-sm" style="margin-left: var(--spacing-md); color: rgba(255, 255, 255, 0.7);">
                        ${item.changes
  ...
  let html = `
        <div class="glass-card p-xl bg-success-light border-left-success">
            ...
            <div class="text-white text-lg" style="line-height: 1.8;">
                ...
                )}개</strong>의 거래처 데이터가 데이터베이스에 저장되었습니다.</p>
                ${
...
function showValidationErrors(validation) {
  ...
  if (validation.errors.length > 20) {
    html += `<div class="mt-sm color-warning">... 외 ${
...
async function loadUploadHistory() {
  try {
    ...
    document.getElementById("history-list").innerHTML =
      '<div class="no-data">이력을 불러올 수 없습니다.</div>';
  }
}
