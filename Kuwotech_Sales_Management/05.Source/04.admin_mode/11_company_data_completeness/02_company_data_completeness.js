...
console.log('📝 [거래처 데이터관리 - 관리자] v1.0 로드됨');
console.log('[거래처 데이터관리 - 관리자] 전체 회사 데이터 기준으로 동작합니다');
 ...
 */
const init = async () => {
  logger.info('[거래처 데이터관리 - 관리자] 페이지 초기화 시작');
...
const loadRegions = async () => {
  try {
    logger.info('[거래처 데이터관리 - 관리자] 지역 목록 조회 시작');
    const apiManager = ApiManager.getInstance();
    const response = await apiManager.get('/regions');

    if (response && response.success !== false) {
      regions = response.data || response || [];
      logger.info(`[거래처 데이터관리 - 관리자] ${regions.length}개 지역 로드 완료`);
    }
  } catch (error) {
    logger.error('[거래처 데이터관리 - 관리자] 지역 목록 로드 실패:', error);
    showToast('⚠️ 지역 정보를 불러오는데 실패했습니다.', 'warning');
  }
};

/**
 * 데이터 완성도 로드 (관리자 모드 - 전체 회사 데이터)
 */
const loadCompletenessData = async () => {
  try {
    logger.info('[거래처 데이터관리 - 관리자] 전체 회사 데이터 완성도 조회 시작');

    const apiManager = ApiManager.getInstance();

    // ⚠️ 관리자 모드: manager 파라미터를 전달하지 않음 → 전체 회사 데이터
    const params = {};
    logger.info('[거래처 데이터관리 - 관리자] ✅ 전체 회사 데이터 조회 (필터링 없음)');

    const response = await apiManager.get('/companies/data-completeness', params);

    logger.info('[거래처 데이터관리 - 관리자] API 응답:', response);

    if (response && response.success !== false) {
      completenessData = response.data || response;
      logger.info('[거래처 데이터관리 - 관리자] 데이터 완성도 로드 완료');
    ...
    logger.error('[거래처 데이터관리 - 관리자] 데이터 완성도 로드 실패:', error);
...
const renderCompletenessCards = () => {
  ...
  logger.info('✅ [거래처 데이터관리 - 관리자] 카드 렌더링 완료');
};

/**
 * 편집 모달 열기 (관리자 모드 - 전체 회사 데이터)
 */
window.openEditModal = async (fieldKey) => {
  logger.info(`[거래처 데이터관리 - 관리자] 모달 열기: ${fieldKey}`);

  currentField = FIELD_DEFINITIONS.find((f) => f.key === fieldKey);
  if (!currentField) {
    logger.error('[거래처 데이터관리 - 관리자] 필드를 찾을 수 없음:', fieldKey);
    return;
  }

  try {
    // ⚠️ 관리자 모드: manager 파라미터를 전달하지 않음 → 전체 회사의 미완성 데이터
    const params = {
      field: currentField.dbColumn
    };
    logger.info('[거래처 데이터관리 - 관리자] 전체 회사의 미완성 데이터 조회');
    ...
    if (response && response.success !== false) {
      ...
      logger.info(`[거래처 데이터관리 - 관리자] ${incompleteCompanies.length}개 미완성 데이터 로드`);
    ...
    logger.error('[거래처 데이터관리 - 관리자] 미완성 데이터 로드 실패:', error);
...
const autoSetRegion = async (companyId, address) => {
  try {
    logger.info(`[거래처 데이터관리 - 관리자] 자동 지역 설정 시작: ${address}`);
    ...
    if (!match) {
      logger.warn('[거래처 데이터관리 - 관리자] 주소에서 지역을 추출할 수 없음');
      return;
    }
    ...
    logger.info(`[거래처 데이터관리 - 관리자] 추출된 지역 정보: ${customerRegion}`);
    ...
    if (matchedRegion) {
      logger.info(`[거래처 데이터관리 - 관리자] 지역 매칭 성공: ID ${matchedRegion.id}, ${matchedRegion.district}`);
      ...
      logger.info(`[거래처 데이터관리 - 관리자] 지역 정보 업데이트 완료: ${companyId}`);
    } else {
      logger.warn(`[거래처 데이터관리 - 관리자] 매칭되는 지역을 찾을 수 없음: ${customerRegion}`);
    ...
    logger.error('[거래처 데이터관리 - 관리자] 자동 지역 설정 실패:', error);
  }
};
...
const handleSave = async () => {
  try {
    logger.info('[거래처 데이터관리 - 관리자] 저장 시작');
    ...
    logger.info(`[거래처 데이터관리 - 관리자] ${updates.length}개 업데이트 요청`);
    ...
    if (response && response.success !== false) {
      logger.info('[거래처 데이터관리 - 관리자] 업데이트 성공');
    ...
    logger.error('[거래처 데이터관리 - 관리자] 저장 실패:', error);
...
window.addEventListener('pageLoaded', (event) => {
  if (event.detail.page === 'company-data-completeness') {
    logger.info('🔄 [거래처 데이터관리 - 관리자] pageLoaded 이벤트로 재초기화');
    init();
  }
});
