...
function updateDepartmentSelection() {
  const checkboxes = document.querySelectorAll(
    '#department-dropdown-menu input[type="checkbox"]'
  );
  const selectedValues = Array.from(checkboxes)
    .filter((cb) => cb.checked)
    .map((cb) => cb.value);
...
function updateEmployeeSelection() {
  const checkboxes = document.querySelectorAll(
    '#employee-dropdown-menu input[type="checkbox"]'
  );
  const selectedValues = Array.from(checkboxes)
    .filter((cb) => cb.checked)
    .map((cb) => cb.value);
  ...
  try {
    ...
    if (currentFilter.name) {
      companyList = companyList.filter(
        (c) => getCompanyDisplayName(c) === currentFilter.name
      );
    }
        ...
        <div class="company-detail">
            <div class="detail-group">
                ...
                <div class="detail-item">
                    <label>연락처:</label>
                    <span>${
                      company.contact || company.phoneNumber || "-"
                    }</span>
                </div>
                <div class="detail-item">
                    <label>주소:</label>
                    <span>${
                      company.address || company.detailedAddress || "-"
                    }</span>
                </div>
            </div>
...
async function openCompanyModal() {
  ...
  try {
    ...
    if (employeesResponse.ok) {
      const employeesData = await employeesResponse.json();
      if (employeesData.success) {
        employeesHtml += employeesData.employees
          .map(
            (emp) =>
  ...
  try {
            ...
            .map(
              (emp) =>
                `<option value="${emp.name}" ${
                  emp.name === company.internalManager ? "selected" : ""
                }>${emp.name} (${emp.department || ""})</option>`
            )
            .join("");
        }
      }
    ...
    const modalContent = `
            <div class="company-form">
                ${
                  !isAdmin
                    ? '<div class="permission-warning">⚠️ 읽기 전용 모드입니다. 수정 권한이 없습니다.</div>'
                    : ""
                }
                <div class="form-section">
                    <h4>📝 기본 정보</h4>
                    <div class="form-grid">
                        <div class="form-group">
                            <label class="required">최종거래처명</label>
                            <input type="text" id="modal-final-company-name" class="form-control" value="${
                        ...
                        <div class="form-group">
                            ...
                            }" placeholder="000-00-00000" ${readonlyAttr}>
                        </div>
                        <div class="form-group">
                            <label class="required">대표이사/치과의사</label>
                        ...
                        <div class="form-group">
                            ...
                            <input type="tel" id="modal-phone-number" class="form-control" value="${
                              company.phoneNumber || ""
                            }" placeholder="02-0000-0000" ${readonlyAttr}>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="modal-last-payment-amount" class="form-control" value="${
                              company.lastPaymentAmount
                                ...
                                : "0"
                            }" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/\\B(?=(\\d{3})+(?!\\d))/g, ',')" ${disabledAttr}>
                        </div>
                        <div class="form-group">
                            <label>누적수금금액</label>
                            <input type="text" class="form-control readonly-bg" value="${
              ...
              onClick: async () => {
                ...
                try {
                  ...
                  if (!response.ok) {
                    const errorData = await response.json();
                    throw new Error(
                      errorData.message || "거래처 삭제에 실패했습니다."
                    );
                  }
                  ...
                  logger.error("[거래처 삭제 실패]", error);
                  showToast(
                    error.message || "거래처 삭제 중 오류가 발생했습니다.",
                    "error"
                  );
                  return null; // 모달 유지
                }
            ...
            },
            {
              ...
              onClick: async () => {
                ...
                const updatedData = {
                  ...
                  phoneNumber:
                    document
                      .getElementById("modal-phone-number")
                      ?.value.trim() || null,
                  isClosed:
                  ...
                  jcwContribution:
                    ...
                    null,
                  companyContribution:
                    document.getElementById("modal-company-contribution")
                      ?.value || null,
                  detailedAddress:
                    document
