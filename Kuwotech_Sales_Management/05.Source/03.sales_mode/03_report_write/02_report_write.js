...
function initializeFlatpickr() {
  ...
  if (typeof window.flatpickr !== "undefined") {
    ...
    logger.warn(
      "[Report Write] ⚠️ Flatpickr 라이브러리를 찾을 수 없습니다 - 기본 date input 사용"
    );
    reportDate.value = today;
    reportDate.setAttribute("type", "date");
    reportDate.removeAttribute("readonly");
...
function cacheElements() {
  ...
  elements.companyAutocompleteList = document.getElementById(
  ...
  // 목표수금금액
  elements.enableTargetCollection = document.getElementById(
    "enableTargetCollection"
  );
  elements.targetCollectionContent = document.getElementById(
    "targetCollectionContent"
  );
  elements.targetCollectionAmount = document.getElementById(
    "targetCollectionAmount"
  );
  elements.targetCollectionCurrency = document.getElementById(
    "targetCollectionCurrency"
  );
  ...
  elements.activityFormContainer = document.getElementById(
...
async function initializePage() {
  try {
    ...
    if (!state.currentUser.id) {
      logger.error(
        "[Report Write] ❌ 사용자 정보에 id가 없습니다:",
        state.currentUser
      );
      if (window.Toast) {
        window.Toast.error(
          "세션 정보가 올바르지 않습니다. 다시 로그인해주세요."
        );
      }
      setTimeout(() => {
...
function attachEventListeners() {
  ...
  // 모달 내 활동 유형 선택
  elements.modalActivityType.addEventListener(
    "change",
    handleModalActivityTypeChange
  );
...
async function loadUserCompanies() {
  try {
    // 담당자별 거래처 조회 API 호출
    const response = await apiManager.getCompaniesByManager(
    ...
    if (response.success) {
      ...
      if (window.Toast) {
        window.Toast.warning(
          `거래처 목록을 불러오지 못했습니다: ${response.message}`
        );
      }
    }
  } catch (error) {
...
async function handleVerifyCompany() {
  ...
  const company = state.companies.find(
    (c) => getCompanyDisplayName(c) === companyName
  );

  if (!company) {
    if (window.Toast) {
      window.Toast.error(
        `❌ 유효하지 않은 거래처입니다: "${companyName}"\n목록에서 선택해주세요.`
      );
    }
    elements.companySelect.focus();
    return;
...
async function loadEmployeeGoals() {
  try {
    ...
    const response = await apiManager.get(
      `/goals/employee/${state.currentUser.id}/monthly`,
      { year, month }
    );
...
async function loadCompanyGoals(companyId) {
  try {
    ...
    const response = await apiManager.get(
      `/goals/company/${companyId}/monthly`,
      { year, month }
    );
...
function updateGoalItem(prefix, data) {
  ...
  // 달성률 표시 (✅ % 소수점 2자리)
  document.getElementById(`${prefix}Rate`).textContent = formatPercent(
    rate / 100,
    2,
    true
  );
...
function addProductRow() {
  ...
  productNameInput.addEventListener("input", (e) =>
    handleProductInput(e, autocompleteList)
  );
  productNameInput.addEventListener("focus", (e) =>
    handleProductFocus(e, autocompleteList)
  );

  // 문서 클릭 시 자동완성 목록 닫기
  document.addEventListener("click", function (e) {
    if (
      !productNameInput.contains(e.target) &&
 ...
 * 제품 자동완성 결과 표시
 */
function displayProductAutocomplete(
  products,
  searchTerm,
  autocompleteList,
  inputElement
  ...
  products.forEach((product, index) => {
    ...
    if (searchTerm) {
      ...
      displayName = product.productName.replace(
        regex,
        '<strong class="search-highlight">$1</strong>'
      );
    }
  ...
  if (searchTerm) {
    ...
    helpItem.style.cssText =
      "background: #f5f5f5; color: #666; font-size: 11px; text-align: center; cursor: default; border-top: 1px solid #e0e0e0;";
...
function collectActivityData(activityType) {
  ...
  switch (activityType) {
    case "call":
      ...
      const callExecutor = document.querySelector(
        'input[name="callExecutor"]:checked'
      )?.value;
    ...
    case "visit":
      ...
      const visitExecutor = document.querySelector(
        'input[name="visitExecutor"]:checked'
      )?.value;
    ...
    case "document":
      ...
      const docExecutor = document.querySelector(
        'input[name="docExecutor"]:checked'
      )?.value;
...
function addActivityToList(activityData) {
  ...
  switch (activityData.type) {
    ...
    case "visit":
      ...
      break;
    case "document":
      summary = `${activityData.date} - ${activityData.name} (${
        activityData.method === "visit" ? "방문" : "이메일"
      })`;
      break;
    case "other":
      summary =
        activityData.content.substring(0, 50) +
        (activityData.content.length > 50 ? "..." : "");
      break;
  }
...
async function handleSubmit(event) {
  ...
  try {
    ...
    elements.submitBtn.innerHTML =
      '<span class="btn-icon">⏳</span><span>저장 중...</span>';
    ...
    if (response.success) {
      ...
      // KPI 재계산 트리거
      kpiManager.triggerKpiRecalculation();
...
function validateForm() {
  ...
  if (!state.isCompanyVerified) {
    if (window.Toast) {
      window.Toast.warning(
        '⚠️ 거래처 확인을 먼저 해주세요. "✅ 확인" 버튼을 클릭하세요.'
      );
    }
    elements.companySelect.focus();
    return false;
  ...
  if (hasTargetCollection) {
    const amount =
      parseFloat(
        String(elements.targetCollectionAmount.value).replace(/,/g, "")
      ) || 0;
    if (!amount || amount <= 0) {
      if (window.Toast) {
  ...
  if (hasTargetSales) {
    const products =
      elements.salesProductList.querySelectorAll(".product-item");
    if (products.length === 0) {
      if (window.Toast) {
    ...
    for (const product of products) {
      ...
      const amount =
        parseFloat(
          String(product.querySelector(".product-amount").value).replace(
            /,/g,
            ""
          )
        ) || 0;
...
async function collectFormData() {
  ...
  if (elements.enableTargetSales.checked) {
    ...
    for (const product of products) {
      ...
      const vatIncluded = product.querySelector(
        ".product-vat-included"
      ).checked;

      // 제품이 데이터베이스에 존재하는지 확인 (대소문자 구분 없이)
      const existingProduct = state.products.find(
        (p) => p.productName.toLowerCase() === normalizedName.toLowerCase()
      );
      ...
      if (!existingProduct) {
        try {
          ...
          logger.error(
            `[Report Write] 제품 추가 실패: ${normalizedName}`,
            error
          );
  ...
  if (elements.enableActivity.checked) {
    const activityItems =
      elements.activityList.querySelectorAll(".activity-item");
    data.activities = [];
