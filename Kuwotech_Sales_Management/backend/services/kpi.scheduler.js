...
export function startKpiScheduler() {
    ...
    const schedule = cron.schedule('0 0 * * *', async () => {
        console.log('[KPI Scheduler] 자동 KPI 계산 시작 - ' + new Date().toISOString());
        
        try {
            // 1. 모든 영업담당자 KPI 재계산
            const salesResult = await refreshAllSalesKPI();
            console.log(`[KPI Scheduler] 영업담당 KPI 재계산 완료 - ${salesResult.count}명`);

            // 2. 전사 KPI 재계산
            const adminResult = await refreshAdminKPI();
            console.log('[KPI Scheduler] 전사 KPI 재계산 완료');

            console.log('[KPI Scheduler] 자동 KPI 계산 성공 완료 - ' + new Date().toISOString());
        } catch (error) {
    ...
    console.log('[KPI Scheduler] 스케줄러 시작됨 - 매일 자정(00:00 KST)에 KPI 자동 계산');
