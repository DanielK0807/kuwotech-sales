...
app.post('/api/debug/import-employees', async (req, res) => {
  try {
    ...
    if (!existsSync(excelPath)) {
      return res.status(404).json({ error: 'Excel file not found', path: excelPath });
    }
    ...
    for (let i = 0; i < employeeData.length; i++) {
      ...
      try {
        ...
        results.imported++;
        results.samples.push({ name, id: employeeId, affectedRows: result.affectedRows });
      } catch (error) {
        results.errors.push({ row: i, name: row['성명'], error: error.message, stack: error.stack });
      }
    }
...
app.get('/api/debug/employees', async (req, res) => {
  try {
    ...
    const [employeeList] = await db.execute('SELECT id, name, role1, role2, department FROM employees LIMIT 5');
    ...
    res.json({
      database: {
        ...
        schema: schema.map(col => ({ field: col.Field, type: col.Type, key: col.Key }))
      },
      excel: {
        path: excelPath,
        exists: excelExists,
...
app.get('/api/debug/employees-by-role-test/:role', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      steps.push({ step: 2, action: 'Check role columns', error: error.message, success: false });
      throw error;
    }
    ...
    try {
      ...
      steps.push({ step: 3, action: 'Count employees with role', error: error.message, success: false });
      throw error;
    }
    ...
    try {
      ...
      steps.push({ step: 4, action: 'Execute main query', error: error.message, stack: error.stack, success: false });
      throw error;
    }
    ...
    try {
      ...
      steps.push({ step: 5, action: 'Format employee data', error: error.message, success: false });
      throw error;
    }
...
app.get('/api/debug/employees-check', async (req, res) => {
  try {
    ...
    // Get all employees
    const [employees] = await db.execute('SELECT id, name, email, role1, role2, department, status FROM employees LIMIT 20');
